...
The stamper handles project name, repo URL, tagline, package scope, `--blue-*` CSS prefix, and logo substitution. Post-stamp mechanical check fails on any unexpected residual `blueprint-platform` strings.

## Substrate-specific extensions

Some surfaces in this scaffold are substrate-aware (Hive, state-derive, BigCommerce governance views) and are NOT generic Tier-1 features:
...
| Content paths in `src/lib/content.ts` (`PRD.md`, `BRD.md`, `STRATEGY.md`, etc.) | Hard-coded to bc-subs document filenames | Initiative may override by editing the doc-name list; future ADR to parameterize via `blueprint.yml` |
