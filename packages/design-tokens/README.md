...
Canonical design tokens for the blueprint-platform family — OKLCH color, type scale, spacing, motion. Framework-agnostic; consumed by React (`packages/ui`), Svelte (`apps/storefront-svelte`), vanilla HTML (`apps/demos`), and any future surface that joins the family.
...
.brand-tint {
  ...
  background: oklch(from var(--blue-brand-lch) 0.97 0.03 h);
}
```

Change `--blue-brand-lch` once and every derived shade re-themes automatically. The same idiom is used in BigCommerce's storefront-kit; we intentionally rhyme.
...
.headline {
  color: oklch(var(--blue-foreground-lch));
  font-family: var(--blue-font-heading);
  font-size: 2.5rem;
}
```
...
All variables are prefixed `--blue-*` so they never collide with merchant-supplied tokens on the storefront, BigDesign tokens in the admin marketplace shell, or BC platform tokens elsewhere.
