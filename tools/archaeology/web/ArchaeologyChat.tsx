 ...
 *
 * INTEGRATION (drop-in for any React-capable portal):
 *
 *   1. Copy into your portal's component directory
 *      (e.g. apps/portal/src/components/ArchaeologyChat.tsx).
 *   2. scaffold.sh substitutes blueprint + biq below
 *      at install time so the component points at this project's Worker.
 *   3. Mount as a global island in your portal layout (passing pageContext):
 *
 *        <ArchaeologyChat client:idle pageContext={currentPath} />
 *
 *      Optionally pass `getSuggestions` to customize empty-state questions
 *      per page.
 *
 * Styling: assumes Tailwind classes from a design system that exposes
 * `bg-brand`, `text-brand`, `bg-background`, `border-contrast-*` etc.
 * Adjust to your project's tokens (or replace classes wholesale).
 */

const WORKER_URL = 'https://blueprint-archaeology.biq.workers.dev';
...
export interface ArchaeologyChatProps {
  pageContext: string;
  /**
   * Override the default empty-state question suggestions. Receives the
   * current pageContext and returns 2-5 starter questions to display.
   * Falls back to a generic substrate-question set if not provided.
   */
  getSuggestions?: (pageContext: string) => string[];
}

export function ArchaeologyChat({ pageContext, getSuggestions }: ArchaeologyChatProps) {
          ...
          >
            {messages.length === 0 && (
              <EmptyState
                ...
                getSuggestions={getSuggestions ?? defaultSuggestions}
...
/**
 * Generic empty-state suggestions. Returns substrate-agnostic starter
 * questions that work for any project. Override via the `getSuggestions`
 * prop on <ArchaeologyChat /> to surface project-specific prompts per page.
 *
 * Example override:
 *
 *   function getMySuggestions(pageContext: string): string[] {
 *     const baseline = ['What ADRs were ratified this week?'];
 *     if (pageContext.startsWith('/inspect/gates')) {
 *       return ['How does the gating discipline work?', ...baseline];
 *     }
 *     return baseline;
 *   }
 *
 *   <ArchaeologyChat pageContext={path} getSuggestions={getMySuggestions} />
 */
function defaultSuggestions(_pageContext: string): string[] {
  return [
    'What were the most recent architectural decisions?',
    'Which inputs informed the design of this project?',
    'What approaches did we consider and reject, and why?',
